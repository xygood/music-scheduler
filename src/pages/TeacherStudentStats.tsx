...
import { Users, User, ChevronDown, ChevronRight, Filter, Search, Building, Download } from 'lucide-react';
...
const TeacherStudentStats: React.FC = () => {
  ...
  const calculateTeacherStats = (): TeacherStats[] => {
    ...
    // 构建教师ID和姓名的映射
    const teacherIdMap = new Map<string, typeof teachers[0]>();
    const teacherNameMap = new Map<string, typeof teachers[0]>();
    teachers.forEach(t => {
    ...
    teachers.forEach(teacher => {
      ...
      // 教师ID匹配：同时检查 id 和 teacher_id（工号）
      ...
      students.forEach(student => {
        ...
        const isPrimaryMatch = (primaryTeacherId && teacherIds.includes(primaryTeacherId)) ||
                               (primaryTeacherName && primaryTeacherName === teacherName);
        ...
        const secondary1Id = student.assigned_teachers?.secondary1_teacher_id;
        const secondary1Name = student.assigned_teachers?.secondary1_teacher_name;
        const secondary2Id = student.assigned_teachers?.secondary2_teacher_id;
        const secondary2Name = student.assigned_teachers?.secondary2_teacher_name;
        const secondary3Id = student.assigned_teachers?.secondary3_teacher_id;
        const secondary3Name = student.assigned_teachers?.secondary3_teacher_name;
        
        const secondary1Match = (secondary1Id && teacherIds.includes(secondary1Id)) ||
                                   (secondary1Name && secondary1Name === teacherName);
        const secondary2Match = (secondary2Id && teacherIds.includes(secondary2Id)) ||
                                   (secondary2Name && secondary2Name === teacherName);
        const secondary3Match = (secondary3Id && teacherIds.includes(secondary3Id)) ||
                                   (secondary3Name && secondary3Name === teacherName);
  ...
  const handleExport = () => {
    ...
    teacherStats.forEach(teacherStat => {
      const teacherName = teacherStat.teacher.name;
      
      Object.values(teacherStat.primaryStats).forEach(classStats => {
        classStats.forEach(cs => {
          cs.students.forEach(student => {
            exportData.push({
              '教师姓名': teacherName,
              '学生类型': '主项',
              '学生姓名': student.name,
      ...
      Object.values(teacherStat.secondaryStats).forEach(classStats => {
        classStats.forEach(cs => {
          cs.students.forEach(student => {
            exportData.push({
              '教师姓名': teacherName,
    ...
    exportUtils.exportToExcel(exportData, '教师学生分配统计', '教师学生统计');
  };
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden" ref={containerRef}>
          ...
          <div className="p-6 bg-white border-b border-gray-200">
            <div className="flex items-center gap-2 mb-4">
              <h3 className="text-xl font-normal text-gray-800 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              ...
              >
                {/* 教研室标题 - 可点击展开/折叠 */}
                <div 
                  className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg font-normal flex items-center justify-between cursor-pointer hover:bg-purple-50 hover:border-purple-200 transition-all shadow-sm"
                  onClick={() => toggleFaculty(faculty)}
                >
                  <span className="text-sm font-normal text-gray-800">{faculty} ({teachersInFaculty.length}人)</span>
                        ...
                        {teachersInFaculty.map(teacherStat => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className={`text-xs font-normal ${activeTeacherId === teacherStat.teacher.id ? 'bg-white text-purple-600' : 'bg-purple-100 text-purple-800'} rounded-full px-2 py-0.5`}>
                                {totalStudents}
                              </span>
                            </button>
                          );
          ...
          {activeTeacherId ? (
                  ...
                  <div className="px-6 py-5 bg-white border-b border-gray-200 shadow-sm">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-xl font-normal text-gray-800">{teacherStat.teacher.name}</h3>
                      ...
                      <div className="flex flex-wrap gap-3">
                        <div className="flex items-center gap-2 bg-blue-50 border border-blue-100 px-4 py-2 rounded-full text-sm shadow-sm">
                          <span className="font-normal text-gray-700">主项:</span>
                          <span className="font-normal text-blue-600">{teacherStat.totalPrimaryStudents}人</span>
                        </div>
                        <div className="flex items-center gap-2 bg-green-50 border border-green-100 px-4 py-2 rounded-full text-sm shadow-sm">
                          <span className="font-normal text-gray-700">副项:</span>
                          <span className="font-normal text-green-600">{teacherStat.totalSecondaryStudents}人</span>
                        </div>
                        <div className="flex items-center gap-2 bg-purple-50 border border-purple-100 px-4 py-2 rounded-full text-sm shadow-sm">
                          <span className="font-normal text-gray-700">总计:</span>
                          <span className="font-normal text-purple-600">{teacherStat.totalPrimaryStudents + teacherStat.totalSecondaryStudents}人</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-6 bg-white">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      ...
                      <div className="bg-white rounded-xl p-5 border border-blue-100 shadow-sm">
                        <h4 className="text-lg font-normal text-gray-800 mb-4 flex items-center gap-2">
                              ...
                              <div key={className} className="bg-white rounded-lg border border-blue-100 overflow-hidden shadow-sm">
                                ...
                                >
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-normal text-gray-800">{className}</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <span className="text-sm font-normal text-blue-500">{classStats[0].students.length}人</span>
                                    {expandedClass && 
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        {classStats[0].students.map(student => (
                                          <div key={student.id} className="flex items-center gap-3 p-3 bg-white rounded-lg hover:bg-blue-100 transition-colors border border-blue-100">
                                              <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center text-white font-normal flex-shrink-0">
                                                {student.name.charAt(0)}
                                              </div>
                                              <div className="flex-1 min-w-0">
                                                <p className="font-normal text-gray-900">{student.name}</p>
                                                <p className="text-xs text-gray-600">{student.student_id}</p>
                                              </div>
                                              <div className="flex-shrink-0">
                                                <span className="px-2 py-1 bg-blue-500 text-white rounded-full text-xs font-normal whitespace-nowrap">
                            ...
                            return (
                              <div key={className} className="bg-white rounded-lg border border-green-100 overflow-hidden shadow-sm">
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span className="text-sm font-normal text-green-500">{classStats[0].students.length}人</span>
                                    {expandedClass && 
                                      expandedClass.teacherId === teacherStat.teacher.id && 
                                      expandedClass.className === className && 
                                        ...
                                        {classStats[0].students.map((student, index) => {
                                          ...
                                          return (
                                            <div key={studentKey} className="flex items-center gap-3 p-3 bg-white rounded-lg hover:bg-green-100 transition-colors border border-green-100">
                                              <div className="w-10 h-10 bg-green-500 rounded-full flex items-center justify-center text-white font-normal flex-shrink-0">
                                                {student.name.charAt(0)}
                                              </div>
                                              <div className="flex-1 min-w-0">
                                                <p className="font-normal text-gray-900">{student.name}</p>
                                              ...
                                              <div className="flex-shrink-0">
                                                <span className="px-2 py-1 bg-green-500 text-white rounded-full text-xs font-normal whitespace-nowrap">
                                                  {secondaryType}
                                                </span>
                                              </div>
                                            </div>
            ...
            <div className="p-10 flex flex-col items-center justify-center border-t border-gray-200 bg-white">
              ...
              <h3 className="text-xl font-normal text-gray-700 mb-3">请选择一位教师</h3>
